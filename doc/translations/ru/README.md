...
Необходимо настроить среду разработки для каждого разработчика   
на его личном компьютере и по крайней мере одну среду разработки предварительно настроенную на удаленном сервере.  
Например, 4 среды разработчиков + 1 на удаленном сервере для тестирования + 1 для руководителя (главного разработчика) проекта  
Или, как вариант, 2 среды разработчиков и одна на удаленном сервере.  
...
_Для всех сред понадобится `nginx` (для разработки и производства), хотя  
в CodeIgniter есть баг с настройкой `base_url`, и хотя   
невозможно установить `homepage` для скриптов react-js в режиме `npm start` ._  
